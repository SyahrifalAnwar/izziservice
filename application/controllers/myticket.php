...
$sql_approvalticket = "SELECT COUNT(A.id_ticket) AS jml_approval_ticket FROM ticket A WHERE A.status = 1";
        ...
        $data['notif_assignment'] = $row_assignmentticket->jml_assignment_ticket;
         ...
         $sql = "SELECT A.status, A.progress,A.tanggal, A.problem_summary,A.problem_detail,A.userfile,A.tanggal_solved,A.reported, A.tanggal_proses, A.tanggal_solved, F.nama AS nama_teknisi, D.nama, C.id_kategori, A.id_ticket, A.tanggal, B.nama_sub_kategori, C.nama_kategori
                FROM ticket A 
